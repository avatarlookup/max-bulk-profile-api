# MAX Bulk Number Profile — `max_profile_batch` | AvatarLookup

Bulk MAX number profile: the avatar plus display name, user id, gender and last-seen time. Pick the country the numbers belong to when you upload.

This is the official AvatarLookup example repository for **one** bulk product, `max_profile_batch`. It is an asynchronous task: upload a file, get a task id immediately, poll the task, and download the result file when it finishes.

- **Product page:** https://avatarlookup.com/products/max_profile_batch
...
| Products per task | 1 (`max_profile_batch`) |
...
- **This product:** https://avatarlookup.com/products/max_profile_batch
